                ...
                if msg.metadata.get("_progress"):
                    if msg.metadata.get("_tool_hint"):
                        blacklist = {
                            ...
                            for name in self.config.channels.tool_hint_blacklist
                            if str(name).strip()
                        }
                        tool_names = [
                            str(name).strip().lower()
                            for name in msg.metadata.get("_tool_names", [])
                            if str(name).strip()
                        ]
                        # Skip sending hints only when every referenced tool is blacklisted.
                        if tool_names and all(name in blacklist for name in tool_names):
                            continue
