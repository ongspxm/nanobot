...
class TestConsolidationDeduplicationGuard:
    ...
    @pytest.mark.asyncio
    async def test_new_clears_session_without_consolidation(self, tmp_path: Path) -> None:
        """/new clears the session without archiving its messages."""
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        from nanobot.providers.base import LLMResponse
        ...
        loop._consolidate_memory = consolidate  # type: ignore[method-assign]
        ...
        assert session_after is not session
